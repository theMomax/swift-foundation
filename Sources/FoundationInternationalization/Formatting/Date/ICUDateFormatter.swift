...
final class ICUDateFormatter {
    ...
    static func cachedFormatter(for format: Date.ISO8601FormatStyle) -> ICUDateFormatter {
        return cachedFormatter(for: .init(format))
    }
...
extension ICUDateFormatter.DateFormatInfo {
    ...
    init(_ format: Date.ISO8601FormatStyle) {
        self.init(localeIdentifier: "en_US_POSIX", timeZoneIdentifier: format.timeZone.identifier, calendarIdentifier: .gregorian, firstWeekday: 2, minimumDaysInFirstWeek: 4, capitalizationContext: .unknown, pattern: format.format, parseLenient: false)
    }
...
extension ICUDateFormatter.DateFormatInfo {
    ...
    static func cachedUpdateSchedule(for format: Date.ISO8601FormatStyle) -> UpdateSchedule {
        return Self.updateScheduleCache.withLock { state in
            let info = Self(format)
            if let schedule = state[info] {
                return schedule
            } else {
                let schedule = format.format.updateSchedule
